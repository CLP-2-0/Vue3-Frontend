	...
	<table class="table">
		<thead>
			<tr class="bg-light text-left">
				...
				<th class="py-3" scope="col">Grammar</th>
				<th class="py-3" scope="col">Meaning</th>
			</tr>
		</thead>
		<tr v-for="(group, index) in gralist" :key="index" class="border-bottom">
			<td class="p-3">{{ group.num }}</td>
			<td class="p-2">{{ group.structure }}</td>
			<td class="p-2">
				<input type="text" class="grammar-meaning" v-model="group.meaning" :disabled="!editable" />
			</td>
		</tr>
	</table>
	...
	export default {
		...
		methods: {
			...
			async saveMeanings() {
				...
				await LessonApis.saveLessonGrammarMeanings(this.lessonIdx, this.gralist).then(
					async (res) => {
			...
			testing() {
				...
				let list = Array.from(superscriptMap, ([num, structure]) => ({
					num,
					structure,
					meaning: '',
				}));
